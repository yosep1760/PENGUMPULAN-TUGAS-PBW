...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // PERUBAHAN: Insert ke tabel Pesanan menggunakan fungsi CURRENT_DATE() dari MySQL
        $stmt = $conn->prepare("INSERT INTO Pesanan (Tanggal_Pesanan, Pelanggan_ID, Total_Harga) VALUES (CURRENT_DATE(), ?, ?)");
        
        // PERUBAHAN: bind_param disesuaikan menjadi "id" (integer, double) karena tanggal sudah otomatis
        $stmt->bind_param("id", $pelanggan_id, $total_harga);
        $stmt->execute();
